...
(fail-closed, kein Mock) -> Snapshot atomar schreiben ->
diff.diff_snapshots(previous, current) -> erst danach analyze/filter/facts/
LLM. Persistenz nur ueber das Snapshot-Modul (kein update_config).
...
def run(mode: str, dry_run: bool = False) -> int:
    ...
    try:
        if dry_run:
            ...
            # Neuen Stand atomar als Snapshot persistieren (inkl. Strategie-
            # Hash/Content), dann Diff gegen den Vorgaenger berechnen —
            # erst danach analyze/filter/facts.
            current = snapshot.capture(
                portfolio, transactions, mode=mode, sc_meta={"source": "sc"}, strategy=strategy
            )
            changes = diff.diff_snapshots(previous, current["snapshot"])
            logging.info(f"snapshot captured (previous: {current['previous'] is not None})")
